let img_folder_path = "" //画像フォルダのパス
let img_name= "" //画像フォルダ内の画像名
...
document.getElementById("confirm-btn").addEventListener("click", function(){
    ...
    img_folder_path = document.getElementById("img_folder_path").value.toString()
    column_name = document.getElementById("column_name").value
    document.getElementById("imageNumber").value=img_num
    console.log(img_folder_path)
    // document.querySelector(".slide-show img").src=img_folder_path + "/" + folder_name +"_"+ img_num+ ".jpg"
    //画像枚数取得, csvファイル作成
    fetch('/select', {
        method: "POST",
        headers: { "Content-Type": "application/json"},
        body: JSON.stringify({"csv_path": csv_path,"img_folder_path":img_folder_path})
      }).then(
        ...
        json => {
            ...
            document.querySelector(".slide-show img").src="./static/img/" + img_name + "/" + img_name +"_"+ img_num+ ".jpg"
            Swal.fire({
                icon: 'success',
                title: '送信完了',
